...
{
	public static class FickleExpression
	{
		...
		{
			return Expression.Variable(new FickleType(type), name);
		}
		...
		{
			return Expression.Parameter(new FickleType(type), name);
		}
		...
		{
			return Call(null, new FickleType(type), typeof(void), methodName, arguments, true);
		}

		public static MethodCallExpression StaticCall(string type, Type returnType, string methodName, object arguments)
		{
			return Call(null, FickleType.Define(type), returnType, methodName, arguments, true);
		}
		...
		{
			return Call(instance, instance.Type, new FickleType(returnType), methodName, arguments, false);
		}
		...
		private static Tuple<ParameterInfo[], Expression[]> GetParametersAndArguments(object arguments)
		{
			...
			FickleParameterInfo[] parameterInfos;

			if (arguments == null)
			{
				parameterInfos = new FickleParameterInfo[0];
			...
			else if (arguments.GetType().IsAnonymousType())
			{
				...
				parameterInfos = new FickleParameterInfo[properties.Length];
				...
				foreach (var propertyInfo in properties)
				{
					...
					parameterInfos[i] = new FickleParameterInfo(parameterType, parameterName);
			...
			{
				parameterInfos = new FickleParameterInfo[]
				{
					new FickleParameterInfo(((Expression)arguments).Type, "arg1")
				};
			...
			else
			{
				parameterInfos = new FickleParameterInfo[]
				{
					new FickleParameterInfo(arguments.GetType(), "arg1")
				};
		...
		{
			return FickleExpression.Property(instance, FickleType.Define(typeName), propertyName);
		}
		...
		{
			return Expression.Convert(value, new FickleType(type));
		}
